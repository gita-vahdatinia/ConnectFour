...
class AIPlayer:
    ...
    def get_alpha_beta_move(self, board):
        ...
        def alphabeta( board, depth, alpha, beta):
            v = max_value(board,-10000000, +10000000,3)
            values.append(v)
            return v
        ...
        def max_value(board,alpha, beta, depth):
            ...
            for row, col in valid_moves:
                ...
                result = min_value(board,alpha,beta,depth-1)
                v = max(v, result)
                board[row][col] = 0
                if v >= beta:
                    return v
        ...
        def min_value(board,alpha,beta,depth):
            ...
            for row,col in valid_moves:
                board[row][col] = self.player_number
                result = max_value(board, alpha, beta, depth-1)
                v = min (v, result)
                board[row][col] = 0
                if v<= alpha:
                    return v
                b = min(beta,v)
            return v
        return (alphabeta(board, 3, -100000,+100000)) 
